...
| `contrast-check.mjs` | Selected-pair opaque-color contrast CLI with one canonical named JSON batch, compatible focused input, and unchanged pass/fail/input-error exits. |
| `slop-scan.mjs` | Scoped HTML/JavaScript/TypeScript risk scan with normal managed/generated/vendor exclusions, explicit targeting, stable finding fingerprints, full-root/partial-selector labels, and distinct pass/finding/error/not-applicable exits. |
...
| `guidance-regressions.test.mjs`, `hardening-regressions.test.mjs` | Single-owner routed-guidance checks plus provider-free fixtures for untracked output, masked failures, bounded builds, and dependency reproducibility. |

All shipped utilities use the package's Node runtime; no Python or Bash runtime is required. Their canonical consumer invocations are `node agent-rules/tools/slop-scan.mjs --root .`, `node agent-rules/tools/contrast-check.mjs --batch contrast-pairs.json`, and `node agent-rules/tools/file-size-guard.mjs --check src/app.js src/view.tsx`. Each utility prints nonempty evidence and distinguishes checked, advisory/failure, input-error, and not-applicable outcomes. Heuristic warnings require inspection, and sensitive behavior still requires contextual review; an unchanged retry does not strengthen the result.
