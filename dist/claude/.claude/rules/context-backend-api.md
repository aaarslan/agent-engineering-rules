...
Extends security and boundaries with server specifics.
...
- Keep route handlers to the thin role defined in boundaries: validate, authorize, call domain logic, map the result to a response.
...
- Avoid N+1 queries and unbounded responses: batch or join, paginate every list endpoint. See performance.
