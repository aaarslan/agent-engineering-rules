...
- Use `agent-rules/tools/slop-scan.mjs` only when supported web files and relevant risks such as unsafe sinks, timers, storage, dead exports, or TODO residue are present. A partial-selector result is advisory for that selection, not a whole-app or clean result.
